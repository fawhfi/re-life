...
class AuthThemeTests(unittest.TestCase):
    ...
    def test_dark_auth_themes_invert_shared_logo_colors(self):
        theme = Path("static/css/theme.css").read_text(encoding="utf-8")

        for template_name in ("login.html", "register.html"):
            html = Path("templates", template_name).read_text(encoding="utf-8")
            self.assertIn('class="login-logo"><img', html)

        self.assertIn('[data-theme="dark"] .login-logo img,', theme)
        self.assertIn('[data-theme="midnight"] .login-logo img {', theme)
        self.assertIn('filter: invert(1) hue-rotate(180deg);', theme)
