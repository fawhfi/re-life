...
class RecordScopeTests(unittest.TestCase):
    def test_records_load_waits_for_account_init(self):
        ...
        self.assertNotIn("userId: record.userId", records)
        self.assertNotIn("userName: record.userName", records)
    ...
    def test_nav_drag_commits_once_on_release(self):
        ...
        self.assertIn("function getNavInset()", source)
        self.assertIn("getPropertyValue('--nav-indicator-inset')", source)
        self.assertIn("const baseLeft = edge;", source)
        self.assertNotIn("const baseLeft = 5;", source)
    ...
    def test_record_badge_classes_use_only_canonical_modes(self):
        source = Path("static/js/app-records.js").read_text(encoding="utf-8")

        self.assertIn("function canonicalizeRecordMode", source)
        self.assertNotIn("record-card-badge--${r.mode}", source)
        self.assertEqual(source.count("record-card-badge--${badgeMode}"), 2)

    def test_record_requests_use_session_cookie_without_identity_parameters(self):
        source = Path("static/supabase.js").read_text(encoding="utf-8")
        records = Path("static/js/app-records.js").read_text(encoding="utf-8")

        self.assertNotIn("function fallbackUserId()", source)
        self.assertNotIn("function fallbackUserName()", source)
        self.assertIn("async getItems()", source)
        self.assertIn('const data = await requestJson("/api/records");', source)
        self.assertIn("async clearAllItems()", source)
...
class RecordInsertTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_add_item_requires_authenticated_owner_argument(self):
        with self.assertRaises(TypeError):
            await data.add_item({"mode": "dispose", "name": "Bottle"})
