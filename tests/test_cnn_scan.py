...
class CnnScanTests(unittest.TestCase):
    ...
    def setUp(self):
        context = sessions.SessionContext(
            session_id="cnn-scan-session",
    ...
    def test_scan_requires_authentication_before_multipart_parse_or_analysis(self):
        self.session_mock.return_value = None
        form_mock = AsyncMock(side_effect=AssertionError("multipart must not be parsed"))
        analyze_mock = AsyncMock(side_effect=AssertionError("image must not be analyzed"))

        with patch.object(main.Request, "form", new=form_mock), \
             patch.object(main, "analyze_scan_image", new=analyze_mock):
        ...
        self.rate_limit_mock.assert_not_awaited()
