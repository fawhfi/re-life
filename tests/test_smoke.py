...
class SmokeTests(unittest.TestCase):
    ...
    def test_pages_render_and_keep_gsap(self):
        context = SessionContext("smoke-session", {"id": 1}, False)
        with patch(
            "sessions.resolve_session",
            new=AsyncMock(return_value=context),
        ):
            self.client.cookies.set(SESSION_COOKIE_NAME, "smoke-token")
            try:
    ...
    def test_header_permissions_policy_keeps_optional_geolocation(self):
        context = SessionContext("header-session", {"id": 1}, False)
        with patch(
            "sessions.resolve_session",
            new=AsyncMock(return_value=context),
        ):
            self.client.cookies.clear()
            self.client.cookies.set(SESSION_COOKIE_NAME, "header-token")
            try:
