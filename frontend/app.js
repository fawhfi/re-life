...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    requestIdleCallback ? requestIdleCallback(() => {
        loadRewards(); loadFact(); detectCamera();
    }) : setTimeout(() => {
        loadRewards(); loadFact(); detectCamera();
    }, 500);
});
...
async function initAccounts() {
    ...
    if (stored) {
        ...
        const cachedAvatar = safeStorage.get('RE_LIFE_USER_AVATAR') || '👤';
        try {
            ...
            if (user) {
                ...
                if (user.photoUrl) {
                    ...
                    state.userAvatar = cachedAvatar;
                    if (cachedAvatar && cachedAvatar !== '👤') {
                        FB.saveUserData(state.userKey || state.userId, {
                            photoUrl: cachedAvatar,
                            _fallbackName: stored,
                        });
                    }
                }
            } else {
...
async function refreshAccountSnapshot() {
    if (!state.currentUser || typeof FB === 'undefined') return;
    try {
        const user = await FB.getUserByName(state.currentUser);
        if (!user) return;
        let shouldUpdate = false;
        if (user.photoUrl && user.photoUrl !== state.userAvatar) {
            state.userAvatar = user.photoUrl;
            safeStorage.set('RE_LIFE_USER_AVATAR', user.photoUrl);
            shouldUpdate = true;
        }
        const nextSpent = user.spent_points || user.spentPoints || 0;
        const nextEarned = user.earned_points || user.earnedPoints || 0;
        const nextCoupons = user.claimed_coupons || [];
        if (nextSpent !== state.spentPoints) { state.spentPoints = nextSpent; shouldUpdate = true; }
        if (nextEarned !== state.earnedPoints) { state.earnedPoints = nextEarned; shouldUpdate = true; }
        if (JSON.stringify(nextCoupons) !== JSON.stringify(state.claimedCoupons)) {
            state.claimedCoupons = nextCoupons;
            shouldUpdate = true;
        }
        if (shouldUpdate) {
            updateHeaderUI();
...
let accountRefreshHooked = false;
function scheduleAccountRefresh() {
    if (accountRefreshHooked) return;
    accountRefreshHooked = true;
    const refresh = () => {
...
function setAvatar(emoji) {
    ...
    if (state.userKey || state.userId) {
        FB.saveUserData(state.userKey || state.userId, {
            photoUrl: emoji,
            _fallbackName: state.currentUser || '',
        });
    }
    closeModal();
}
    ...
    try {
        ...
        const list = users.map(u => `
            <button class="btn btn--outline btn--full" style="margin-bottom:6px;justify-content:flex-start;gap:8px"
                    onclick='loginAs(${attrEsc(JSON.stringify(u.displayName))}, ${attrEsc(JSON.stringify(u.photoUrl || "👤"))}, ${attrEsc(JSON.stringify(u.id))})'>
                <span style="font-size:20px">${esc(u.photoUrl || '👤')}</span>
...
let tipsRefreshTimer = null;
let tipsRefreshHooked = false;

function scheduleTipsRefresh() {
    if (tipsRefreshHooked) return;
    tipsRefreshHooked = true;
    const refresh = () => {
        if (document.hidden) return;
        loadTips();
    };
    ...
    window.addEventListener('online', refresh);
    window.addEventListener('focus', refresh);
    tipsRefreshTimer = setInterval(refresh, 15 * 60 * 1000);
}
